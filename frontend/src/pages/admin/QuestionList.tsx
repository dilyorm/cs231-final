...
const STATUS_STYLE: Record<string, { label: string; icon: typeof Clock; cls: string }> = {
  ...
  rejected: { label: "Rejected", icon: XCircle, cls: "text-red-500 bg-red-50" },
};
        ...
        >
          ...
          {topics.map(([num, name]) => (
            <option key={num} value={num}>
              T{String(num).padStart(2, "0")} · {name.split(" – ")[1] ?? name}
            </option>
          ))}
        </select>
        {isAdmin && (
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {Object.entries(grouped)
            ...
            .map(([topicNum, qs]) => (
              <div key={topicNum} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                <div className="px-5 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-mono font-bold text-gray-400">
                      T{String(Number(topicNum)).padStart(2, "0")}
                    </span>
                    <span className="text-sm font-semibold text-gray-700">{qs[0].topic}</span>
                  </div>
                  ...
                  {qs.map((q) => {
                    ...
                    return (
                      <div key={q.id}>
                        ...
                        {expanded === q.id && (
                          <div className="px-12 pb-4 space-y-3 bg-gray-50/50">
                            <div>
                            ...
                            {q.code_challenge && (
                              <div>
                                ...
                                <p className="text-sm text-gray-600 bg-white border border-emerald-100 rounded-lg p-3">
                                  {q.code_challenge}
                                </p>
                              </div>
                            )}
